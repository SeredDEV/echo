...
const ProductModal = ({ product, isOpen, onClose }: ProductModalProps) => {
  ...
  const media: MediaItem[] = [
    ...
    ...(product.videos?.map(
      (vid) =>
        ({ type: "video", url: vid, thumbnail: product.imagen } as MediaItem)
    ) || []), // Use main image as video placeholder thumbnail
  ];
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto no-scrollbar animate-fadeInUp flex flex-col">
        ...
        <div className="grid md:grid-cols-2 gap-0 flex-1">
              ...
              <div className="flex flex-col gap-3 overflow-y-auto max-h-[500px] w-24 hide-scrollbar py-2 px-1">
                {media.map((item, index) => (
                  <button
                    key={index}
                    onMouseEnter={() =>
                      item.type === "image" && setActiveMediaIndex(index)
                    }
                    onClick={() => setActiveMediaIndex(index)}
                    className={`relative flex-shrink-0 w-16 h-16 rounded-lg border-2 bg-white overflow-hidden transition-all group ${
                      index === activeMediaIndex
                        ? "border-[#0FA6D1] ring-2 ring-[#0FA6D1]/20 ring-offset-1"
          ...
          <div className="p-6 md:p-8 flex flex-col h-full bg-white overflow-y-auto">
            <div className="">
              <span className="text-[#0FA6D1] font-bold uppercase text-xs tracking-wider bg-[#0FA6D1]/10 px-3 py-1 rounded-full mb-3 inline-block">
                ...
                product.coloresDisponibles.length > 0 && (
                  <div className="mb-6">
                    <h3 className="text-sm font-semibold text-gray-900 mb-3 block">
                      Color seleccionado:{" "}
                      <span className="text-[#0FA6D1] font-bold">
                        {selectedColor}
                      </span>
                    </h3>
                        ...
                        return (
                          <button
                            ...
                            className={`group relative w-12 h-12 rounded-full flex items-center justify-center transition-all ${
                              isSelected
                                ? "ring-2 ring-[#0FA6D1] ring-offset-2 scale-105"
                                : "hover:ring-2 hover:ring-gray-300 hover:ring-offset-1"
                            }`}
                            title={color}
                          >
                            <div
                              className={`w-full h-full rounded-full shadow-sm ${getColorBg(
                                color
                              )}`}
                            ></div>
                          </button>
              ...
              <div className="mt-auto pt-6">
                ...
                <div className="flex gap-3">
                  ...
                  <div className="w-32">
                    <div className="flex items-center justify-between bg-gray-50 rounded-xl border border-gray-200 p-1">
                      <button
                        ...
                        className="w-10 h-10 flex items-center justify-center text-gray-500 hover:text-[#0FA6D1] hover:bg-white rounded-lg transition-all disabled:opacity-30 disabled:hover:bg-transparent"
                      >
                        <svg
                          className="w-4 h-4"
                          fill="none"
                  ...
                  <button
                    ...
                    className="flex-1 bg-gradient-to-r from-[#0FA6D1] to-[#5EA4CF] hover:from-[#0d8bb5] hover:to-[#4d8fb8] text-white font-bold py-4 rounded-xl shadow-xl hover:shadow-2xl hover:scale-[1.01] active:scale-[0.99] transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 group"
                  >
                    <svg
                  ...
                  {Object.entries(product.especificaciones).map(
                    ([key, value]) => {
                      ...
                      const getIcon = (k: string) => {
                        k = k.toLowerCase();
                        if (k.includes("audio") || k.includes("sonido"))
                          return (
                            <svg
                              className="w-5 h-5"
                              fill="none"
                      ...
                      return (
                        <div
                          key={key}
                          className="bg-gray-50 rounded-xl p-4 border border-gray-100 flex flex-col gap-2 hover:bg-white hover:shadow-md transition-all"
                        >
                          <div className="flex items-center gap-2 text-[#0FA6D1] mb-1">
                            {getIcon(key)}
