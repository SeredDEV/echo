          ...
          <div className="relative rounded-2xl md:rounded-3xl overflow-hidden shadow-xl md:shadow-2xl mb-4 md:mb-8">
            ...
            <div className="min-h-[400px] md:aspect-video relative bg-black">
              {activeItem.videos.length > 0 ? (
                ...
                <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-[#0FA6D1] to-[#5EA4CF]">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {carouselData.map((item, index) => (
                    <button
                      ...
                      className={`px-6 py-3 rounded-full font-semibold text-sm lg:text-base transition-all duration-300 ${
                        activeIndex === index
                          ? "bg-[#0FA6D1] text-white shadow-lg scale-105"
              ...
              <div className="grid grid-cols-2 gap-2">
                {carouselData.slice(0, 4).map((item, index) => (
                  <button
                    key={item.id}
                    onClick={() => setActiveIndex(index)}
                    className={`px-4 py-2.5 rounded-full font-semibold text-xs transition-all duration-300 ${
                      activeIndex === index
                        ? "bg-[#0FA6D1] text-white shadow-lg"
              ...
              {carouselData.length > 4 && (
                <div className="mt-2">
                  <button
                    ...
                    className={`w-full px-4 py-2.5 rounded-full font-semibold text-xs transition-all duration-300 ${
                      activeIndex === 4
                        ? "bg-[#0FA6D1] text-white shadow-lg"
                        : "bg-gray-100 text-gray-700 hover:bg-gray-200"
                    }`}
            ...
            <div className="space-y-3 md:space-y-4">
              {activeItem.description.map((desc, idx) => (
                <div key={idx} className="flex items-start gap-2 md:gap-3">
                  <div className="flex-shrink-0 w-5 h-5 md:w-6 md:h-6 rounded-full bg-[#0FA6D1]/10 flex items-center justify-center mt-0.5 md:mt-1">
                    <svg
                      className="w-3 h-3 md:w-4 md:h-4 text-[#0FA6D1]"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
